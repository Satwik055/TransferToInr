...
fun TTFDropdownxy(
    ...
    ExposedDropdownMenuBox(
        ...
        ExposedDropdownMenu(
            ...
            if (items.isEmpty()) {
                // Show "No recipients found" message if the list is empty
                DropdownMenuItem(
                    text = {
                        Text(
                            text = "No recipients found",
                            style = textStyle.copy(color = Color.Gray) // Optional: Change color for the message
                        )
                    },
                    onClick = {
                        // Optionally handle click on the message
                        expanded = false
                        focusManager.clearFocus()
                    }
                )
            } else {
                // Render the list of items if not empty
                items.forEachIndexed { index, item ->
                    DropdownMenuItem(
                        text = {
                            Row(
                                ...
                                Text(text = item.first, style = textStyle) // First part of the Pair
                                Text(text = item.second, style = textStyle) // Second part of the Pair
                            }
                        },
                        onClick = {
                            onItemSelected(item)
